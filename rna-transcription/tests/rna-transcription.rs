...
fn test_transcribes_all_dna_to_rna() {
    assert_eq!(
        Ok(dna::RibonucleicAcid::new("UGCACCAGAAUU")),
...
fn handles_partially_invalid_input() {
    assert!(
        dna::DeoxyribonucleicAcid::new("ACGTXXXCTTAA")
            .to_rna()
            .is_err()
    );
}
